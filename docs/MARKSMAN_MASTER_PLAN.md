...
- **11.3.2** Add Gemini 3.1 Flash as a fallback: if Groq is rate-limited (14,400 req/day shared), fall back to `gemini-3.1-flash` for the AI coach call — configure via `AI_PROVIDER_FALLBACK` env var
...
- **18.2.2** AI video analysis: pass the video URL to Gemini 3.7 / 3.8 (vision-capable) with a prompt analyzing the shooter's stance, grip, hold area, and trigger pull visible in the video — returns structured critique in the same format as `AiCoachFinding`
