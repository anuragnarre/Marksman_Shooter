...
const CATEGORY_META: Record<AiCoachCategory, { label: string; color: string; bg: string; icon: string }> = {
  ...
  general:      { label: 'General',      color: '#8892A4', bg: 'rgba(136,146,164,0.08)', icon: '◆' },
};
      ...
      <div className="space-y-8 max-w-4xl">
        ...
        <div className="animate-slide-up relative rounded-xl border border-[#1E2433] bg-[#0E1118] p-6 overflow-hidden">
          ...
          {loading ? (
            ...
            <p className="text-[#4A5568] text-sm">No sessions found. Record a session first.</p>
          ) : (
              ...
              >
                {analyzing ? (
                  <span className="flex items-center gap-2">
                    <span className="w-3.5 h-3.5 border-2 border-[#080A0F]/40 border-t-[#080A0F]
          ...
          {selectedSession && !analyzing && (
            <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-[#1E2433]">
              {[
                ...
                <div key={label}>
                  ...
                  <p className="text-[#F0F4FF] text-sm font-display font-semibold">{value}</p>
                </div>
              ))}
            </div>
          )}
...
function AnalysisResults({ analysis }: { analysis: AiCoachAnalysis }) {
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      {/* ── Overall card ─────────────────────────────────────────── */}
      <div className="relative rounded-xl border border-[rgba(245,166,35,0.25)] bg-[#0E1118] p-6 overflow-hidden">
        ...
        <div className="relative flex items-start gap-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="ml-auto text-[10px] text-[#4A5568] font-display uppercase tracking-widest">
                {analysis.model}
              </span>
            </div>
      ...
      {analysis.prioritizedActions.length > 0 && (
        <div className="rounded-xl border border-[#1E2433] bg-[#0E1118] p-6">
          ...
          <ol className="space-y-3">
            {analysis.prioritizedActions.map((action, i) => (
              <li key={i} className="flex items-start gap-3">
                ...
                <p className="text-[#8892A4] text-sm leading-relaxed">{action}</p>
              </li>
            ))}
          </ol>
        </div>
      ...
      <div className="rounded-xl border border-[rgba(0,229,160,0.2)] bg-[rgba(0,229,160,0.04)] p-5">
        ...
        <p className="text-[#F0F4FF] text-sm leading-relaxed">{analysis.nextSessionFocus}</p>
      </div>
...
function FindingCard({ finding, index }: { finding: AiCoachFinding; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="font-display font-semibold text-[#F0F4FF] text-sm">{finding.title}</p>
          ...
          <p className="text-[#4A5568] text-[11px] mt-0.5 truncate">{finding.observation}</p>
        </div>
      ...
      {/* Expanded body */}
      {open && (
        <div className="px-5 pb-5 space-y-4 border-t border-[#1E2433]/60">
          <div className="pt-4 space-y-3">
            <div>
              <p className="label text-[9px] mb-1">Observation</p>
              <p className="text-[#8892A4] text-sm leading-relaxed">{finding.observation}</p>
            </div>
            <div>
              <p className="label text-[9px] mb-1" style={{ color: cat.color }}>Suggestion</p>
              <p className="text-[#F0F4FF] text-sm leading-relaxed">{finding.suggestion}</p>
            </div>
            {finding.drill && (
              <div className="rounded-lg border border-[rgba(245,166,35,0.2)] bg-[rgba(245,166,35,0.04)] p-3">
                ...
                <p className="text-[#8892A4] text-sm leading-relaxed">{finding.drill}</p>
              </div>
            )}
          </div>
        </div>
