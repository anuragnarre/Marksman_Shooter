          ...
          <div className="card p-6">
            ...
            {sessions.length === 0 && !selectedSessionId && (
              <div className="py-12 flex flex-col items-center text-center">
                ...
                <p className="font-display font-bold text-base text-[#F0F4FF]">No sessions yet</p>
                <p className="text-[#8892A4] text-sm mt-1 max-w-xs">
          ...
          <div className="card p-6">
                  ...
                  {[7, 30, 90].map(d => (
                    <button
                      ...
                      style={{
                        ...
                        color: insightPeriod === d ? '#F5A623' : '#4A5568',
            ...
            {advancedLoading && (
              <div className="mt-6 flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm font-display font-semibold text-[#F0F4FF]">Analyzing your biometric data...</p>
                <p className="text-[10px] mt-1" style={{ color: '#8892A4' }}>Processing {insightPeriod} days of readings across all domains</p>
              </div>
            )}
            ...
            {advancedInsights && (
              <div className="mt-4 space-y-5">
                ...
                <div className="flex items-center gap-5 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <ReadinessGauge score={advancedInsights.overallReadiness} />
                  <div className="flex-1">
                    <p className="text-[10px] font-display font-bold uppercase tracking-wider" style={{ color: '#8892A4' }}>
                      Overall Readiness
                    </p>
                    <p className="font-display font-bold text-lg text-[#F0F4FF]">
                  ...
                  <button
                    ...
                    style={{ color: '#8892A4', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}
                  >
                    Refresh
                  </button>
                </div>
                    ...
                    <div className="space-y-3">
                      {advancedInsights.breathing.estimatedRate !== null && (
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-display font-bold uppercase tracking-wider" style={{ color: '#8892A4' }}>Est. Rate</span>
                          <span className="font-mono text-sm font-bold" style={{ color: '#00E5A0' }}>
                            {advancedInsights.breathing.estimatedRate} <span className="text-[10px] font-normal" style={{ color: '#4A5568' }}>br/min</span>
                          </span>
                        </div>
                      )}
                      <p className="text-xs" style={{ color: '#8892A4' }}>{advancedInsights.breathing.pattern}</p>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-3 gap-2">
                        <div className="text-center">
                          ...
                          <p className="text-[9px] font-display font-bold uppercase" style={{ color: '#4A5568' }}>Resting</p>
                        </div>
                        <div className="text-center">
                          <p className="font-mono text-sm font-bold" style={{ color: '#F5A623' }}>{advancedInsights.hrStability.activeHr}</p>
                          <p className="text-[9px] font-display font-bold uppercase" style={{ color: '#4A5568' }}>Active</p>
                        </div>
                        <div className="text-center">
                          <p className="font-mono text-xs font-bold" style={{ color: '#8892A4' }}>{advancedInsights.hrStability.recoveryRate}</p>
                          <p className="text-[9px] font-display font-bold uppercase" style={{ color: '#4A5568' }}>Recovery</p>
                        </div>
                      </div>
                      {/* Zone bar */}
                      <div>
                        <p className="text-[9px] font-display font-bold uppercase tracking-wider mb-1" style={{ color: '#8892A4' }}>HR Zone Distribution</p>
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-[10px] font-display font-bold uppercase tracking-wider" style={{ color: '#8892A4' }}>HRV Trend</p>
                          <p className="text-xs" style={{ color: '#F0F4FF' }}>{advancedInsights.focusStress.hrvTrend}</p>
                        </div>
                      </div>
                      <div className="space-y-1.5">
                ...
                {advancedInsights.insights.length > 0 && (
                  <div>
                    <h4 className="font-display font-semibold text-sm text-[#F0F4FF] mb-3">
...
function DomainPreview({ icon, title, description, color }: {
  ...
  return (
    <div className="card p-4 text-center transition-all hover:scale-[1.02]" style={{ border: `1px solid ${color}15` }}>
      ...
      <p className="font-display font-semibold text-xs text-[#F0F4FF] mb-1">{title}</p>
      <p className="text-[10px]" style={{ color: '#8892A4' }}>{description}</p>
    </div>
  );
}
    ...
    <div className="card p-5" style={{ border: `1px solid ${color}20` }}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <h4 className="font-display font-semibold text-sm text-[#F0F4FF]">{title}</h4>
        </div>
        {score !== null && (
          <div className="text-right">
            <p className="font-mono text-lg font-bold" style={{ color }}>
              {score}<span className="text-xs font-normal" style={{ color: '#4A5568' }}>/100</span>
            </p>
            <p className="text-[8px] font-display font-bold uppercase tracking-wider" style={{ color: '#4A5568' }}>{scoreLabel}</p>
          </div>
        )}
      </div>
      {children}
...
function RecommendationItem({ text, color }: { text: string; color: string }) {
  return (
    <div className="flex items-start gap-2">
      ...
      <p className="text-[11px]" style={{ color: '#F0F4FF' }}>{text}</p>
    </div>
  );
}
...
function PerfDetail({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start gap-2 p-2 rounded-lg" style={{ background: 'rgba(79,195,247,0.04)' }}>
      <p className="text-[9px] font-display font-bold uppercase tracking-wider shrink-0 w-24 pt-0.5" style={{ color: '#4A5568' }}>{label}</p>
      <p className="text-[11px]" style={{ color: '#F0F4FF' }}>{value}</p>
    </div>
  );
}
    ...
    <div className="card p-4" style={{ borderLeft: `3px solid ${severityColor}` }}>
      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
        ...
        {insight.metric && insight.metricValue && (
          <span className="font-mono text-[10px] font-bold" style={{ color: '#F0F4FF' }}>
            {insight.metricValue}
          </span>
        )}
        {insight.trend && (
      ...
      <p className="text-[11px] mb-1.5" style={{ color: '#8892A4' }}>{insight.observation}</p>
      <p className="text-[11px]" style={{ color: '#F0F4FF' }}>{insight.recommendation}</p>
    </div>
  );
}
