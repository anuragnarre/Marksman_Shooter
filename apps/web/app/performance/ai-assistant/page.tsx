...
const TECHNIQUE_META: Record<string, { label: string; color: string; icon: string }> = {
  ...
  stability:     { label: 'Stability',      color: 'var(--text-secondary)', icon: '\u229A' },
...
const MENTAL_META: Record<string, { color: string; icon: string }> = {
  ...
  meditation:    { color: 'var(--text-secondary)', icon: '\u25CB' },
...
const PHYSICAL_META: Record<string, { color: string; icon: string }> = {
  ...
  recovery:    { color: 'var(--text-secondary)', icon: '\u2726' },
};
      ...
      <div className="space-y-8 max-w-5xl">
        ...
        <div className="animate-slide-up">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="font-display font-bold text-2xl text-text-primary">AI Performance Assistant</h2>
          ...
          {isCoach && selectedShooter && (
            <p className="text-text-muted text-xs mt-2">
              Analysing: <span className="text-text-primary">{selectedShooter.name}</span>
            </p>
          )}
        </div>
        ...
        {!data && !loading && (
          <div className="animate-slide-up">
            <div className="relative rounded-xl border border-border-subtle bg-surface p-8 text-center overflow-hidden">
        ...
        {data && !loading && (
          <div className="space-y-6 animate-slide-up">
            ...
            {/* ── Tab navigation ────────────────────────────────────── */}
            <div className="flex gap-1 p-1 rounded-xl bg-surface border border-border-subtle overflow-x-auto">
              {([
                ...
                <button
                  ...
                  style={{
                    ...
                    color: activeTab === tab.key ? '#F5A623' : 'var(--text-muted)',
...
function LoadingState() {
  ...
  return (
    <div className="rounded-xl border border-[rgba(245,166,35,0.2)] bg-surface p-12
      ...
      {/* Progress bar */}
      <div className="w-64 h-1 rounded-full bg-subtle mt-6 overflow-hidden">
        <div
...
function OverviewCard({ data, onReanalyze }: { data: AiPerformanceAssistant; onReanalyze: () => void }) {
  ...
  return (
    <div className="relative rounded-xl border border-[rgba(245,166,35,0.25)] bg-surface overflow-hidden">
      <div className="absolute top-0 inset-x-0 h-px"
        ...
        <div className="flex items-start gap-6 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-3">
              ...
              <button
                onClick={onReanalyze}
                className="ml-auto text-[10px] font-display font-semibold text-text-muted hover:text-[#F5A623] transition-colors"
              >
                Re-analyse
              </button>
            </div>
...
function TechniqueCard({ insight, tech, status, index }: {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="font-display font-semibold text-text-primary text-sm">{insight.title}</p>
      ...
      {open && (
        <div className="px-5 pb-5 space-y-3 border-t border-border-subtle/60 pt-4">
          <div>
            <p className="label text-[9px] mb-1">Observation</p>
            <p className="text-text-secondary text-sm leading-relaxed">{insight.observation}</p>
          </div>
          <div>
            <p className="label text-[9px] mb-1" style={{ color: tech.color }}>Correction</p>
            <p className="text-text-primary text-sm leading-relaxed">{insight.correction}</p>
          </div>
          {insight.drill && (
            <div className="rounded-lg border border-[rgba(245,166,35,0.2)] bg-[rgba(245,166,35,0.04)] p-3">
              ...
              <p className="text-text-secondary text-sm leading-relaxed">{insight.drill}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="rounded-xl border border-border-subtle bg-surface p-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <p className="font-data font-bold text-xl text-text-primary">{comparison.recentAvg.toFixed(2)}</p>
            <p className="text-[10px] text-text-muted font-display mt-1">Recent Avg</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            <p className="font-data font-bold text-xl text-text-secondary">{comparison.previousAvg.toFixed(2)}</p>
            <p className="text-[10px] text-text-muted font-display mt-1">Previous Avg</p>
          </div>
        </div>
        ...
        <div className="flex justify-between text-[9px] text-text-muted font-display mt-1">
          <span>Previous</span>
          <span>Recent</span>
        </div>
      </div>
            ...
            return (
              <div key={i} className="rounded-xl border border-border-subtle bg-surface p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="font-display font-semibold text-sm text-text-primary">{p.title}</p>
                ...
                <p className="text-text-secondary text-xs leading-relaxed mb-2">{p.detail}</p>
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-display text-text-muted uppercase tracking-widest">{p.type}</span>
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {recommendations.map((rec, i) => {
          ...
          return (
            <div key={i} className="rounded-xl border border-border-subtle bg-surface p-5 overflow-hidden relative">
              <div className="absolute top-0 inset-x-0 h-px"
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <div>
                  <p className="font-display font-semibold text-sm text-text-primary">{rec.title}</p>
              ...
              <p className="text-text-secondary text-xs leading-relaxed mb-3">{rec.description}</p>

              {rec.routine && (
                <div className="rounded-lg bg-[rgba(255,255,255,0.02)] border border-border-subtle p-3 mb-2">
                  <p className="label text-[8px] mb-1" style={{ color: meta.color }}>Routine</p>
                  <p className="text-text-primary text-xs leading-relaxed">{rec.routine}</p>
                </div>
              )}

              {rec.duration && (
                <p className="text-[10px] text-text-muted font-display">
                  Duration: <span className="text-text-secondary">{rec.duration}</span>
                </p>
              )}
            </div>
          );
          ...
          return (
            <div key={i} className="rounded-xl border border-border-subtle bg-surface p-5 overflow-hidden relative">
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                {rec.frequency && (
                  <span className="ml-auto text-[9px] font-display text-text-muted px-2 py-0.5 rounded
                                   bg-[rgba(255,255,255,0.03)] border border-border-subtle">
                    {rec.frequency}
                  </span>
                )}
              </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {rec.exercises.map((ex, j) => (
                    <div key={j} className="flex items-start gap-3">
                      ...
                      <p className="text-text-primary text-xs">{ex}</p>
                    </div>
                  ))}
                </div>
              )}
...
function InsightsTab({ alerts, plan, weaknesses, strengths }: {
  ...
  return (
    <div className="space-y-6">
            ...
            return (
              <div key={i} className="rounded-xl overflow-hidden"
                ...
                <div className="px-5 py-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-base" style={{ color: meta.color }}>{meta.icon}</span>
                    <p className="font-display font-semibold text-sm text-text-primary">{alert.title}</p>
                    <span className="ml-auto text-[9px] font-display font-bold uppercase tracking-widest px-1.5 py-0.5 rounded"
                          style={{ color: meta.color, border: `1px solid ${meta.border}` }}>
                      {alert.severity}
                    </span>
                  </div>
                  <p className="text-text-secondary text-xs leading-relaxed mb-2">{alert.message}</p>
                  <div className="flex items-center gap-2">
      ...
      {/* Improvement Plan */}
      <div className="rounded-xl border border-[rgba(245,166,35,0.25)] bg-surface overflow-hidden">
        <div className="px-5 py-4 border-b border-border-subtle"
          ...
          <p className="text-text-secondary text-xs mt-1">{plan.timeframe}</p>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="rounded-lg border border-[rgba(0,229,160,0.2)] bg-[rgba(0,229,160,0.04)] p-4">
            ...
            <p className="text-text-primary text-sm font-display font-semibold">{plan.goal}</p>
          </div>
...
function SectionHeader({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div>
      <p className="font-display font-bold text-lg text-text-primary">{title}</p>
...
function EmptySection({ label }: { label: string }) {
  return (
    <div className="rounded-xl border border-border-subtle bg-surface p-8 text-center">
      <p className="text-text-muted text-sm">No {label} generated for this analysis.</p>
    </div>
  );
}
