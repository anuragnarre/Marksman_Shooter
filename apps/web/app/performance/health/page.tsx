...
export default function BiometricsPage() {
  ...
  useEffect(() => {
    isHcAvailable().then(setHcAvailable);
    // Load registered devices to show Marksman Pulse badge
    apiFetch<Array<{ deviceType: string; deviceName: string }>>('/biometrics/devices')
      .then(devs => {
        const hc = devs.find(d => d.deviceType === 'HEALTH_CONNECT');
        setDeviceName(hc?.deviceName ?? null);
      })
      .catch(() => {});
  }, []);
  ...
  async function syncAndRefresh() {
    ...
    try {
      ...
      // Re-fetch sessions summary after sync
  ...
  return (
    <AppShell title="Health">
      <div className="space-y-6 max-w-6xl mx-auto">
        ...
        {/* Header with device badge + sync + manage */}
        <div className="flex items-start justify-between animate-slide-up flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-2.5">
              <h2 className="font-display font-bold text-xl text-text-primary">Biometrics</h2>
              {deviceName && (
                <span
                  className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-display font-bold uppercase tracking-wider"
                  style={{
                    color: '#F5A623',
            ...
            <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>
              Real-time heart rate, SpO₂, and physiological tracking
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {hcAvailable && (
              <button
                ...
                style={{
                  color: '#F5A623',
                  background: syncing ? 'rgba(245,166,35,0.05)' : 'rgba(245,166,35,0.1)',
                  border: '1px solid rgba(245,166,35,0.2)',
                }}
              >
                {syncing ? (
                  <>
                    <span className="w-3.5 h-3.5 border border-t-[#F5A623] border-[rgba(245,166,35,0.2)] rounded-full animate-spin" />
                    Syncing...
                  </>
                ) : (
                  <>
